...
propertyCodes.pop()
console.log(propertyCodes)

async function forLoop() {
    for(let k = 0; k < 3; k++) {
        var propertyCode = propertyCodes[k]
        await new Promise(next => { getPrices(propertyCode).then(()=>{console.log('done');
        next() })       
        });
    }
}
...
async function getPrices(propertyCode)  {
    let min = 9999
    ...
    console.log("/////////////////////")
    console.log("Final code to use for " + propertyCode + ": " + useableCode)
    console.log("/////////////////////")
